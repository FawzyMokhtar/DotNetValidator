...
namespace DotNetValidator
{
    public static partial class ValidationUtility
    {
        ...
        public static Validator StartsWith(this Validator validator, string str, string errorMessage = null)
        {
            try
            {
                ...
                if (!validator.IsOptional || value != null)
                {
                    if (!value.ToString().StartsWith(str))
                        validator.AddError(errorMessage ?? DefaultErrorMessages.StartsWith(str));
                }
            }
            catch (Exception)
            {
